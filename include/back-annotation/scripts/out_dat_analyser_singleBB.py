...
#    where addr is the ending address of the basic block.
#    Next look for "ITIME" and record time from the "t=n" phrase in the same
#    line
#4 Oct12 - Making change that was tried on 30 Sep for BB power calc. 
#   trying bbDuration - overlap instead of just bbDuration.
...
#         address of last instruction in the BB/TB
#Output - A file with the cycle count as analysed. File name is input
#         dat file with .metric extension. First line is cycle count,
#         second (when implemented) is energy
##################################################################################
def analyseDat(iDat, endAddr):
  traceSection = "TRACE"
  ...
  endAddrString = instrAdrMark + endAddr
  ...
  timeRecord = 0 #records time from trace
...
#rresenting stats needed by McPAT, only relevant ones for now
  ...
  for codeLine in iFilePtr:
    ...
    ######################## Timing Analysis ##############################
    if endAddrFound == 0: #if this DAT's ending instruction hasn't been found yet
                        #search for it
      m = re.search(endAddrString, codeLine) 
      if m != None: #match
        #if phase == 1: 
    ...
    if m != None: #there is a match
      timeRecord = int( codeLine[m.end():] ) #record time
      resultFilePtr.write( codeLine[m.end():] )
      resultFilePtr.close()
      #if TimeDebug:
