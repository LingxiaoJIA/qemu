                                                  ...
                                                  bool search_pc)
{
    ...
    for(ii=0; ii<TB_targetCode[tb_IDtracker].tbSize; ii++) {
      printf("At pred PC %x opcode is %x\n",
               TB_targetCode[tb_IDtracker].myPCs[ii],
               TB_targetCode[tb_IDtracker].myOpcodes[ii]);
    }
...
/* Modified */
/* Func name - cz_unseenPair
 * Purpose - Characterize a BB for a predecessor that hasn't been
 * on the path to the BB before. Assuming that opcodes for BB and
 * predecessor would have been stored already in the target buffer
...
{
    characterize_TB(
            TB_targetCode[tbID].myOpcodes,  // curr opcodes
            TB_targetCode[tbID].myPCs,      // curr bb PC
            TB_targetCode[tb_IDtracker].myOpcodes,  // pred opcodes
...
/* Func name - characterize_TB
 * Purpose - Create ISS input stimulus for given opcodes and their addresses.
 * Then execute them on the ISS and extract cycle count from output trace
 * Inputs:
                     ...
                     uint32_t hasPred, uint32_t tbID)
{
    ...
    */
    ...
    for (i = 0; i < tbSize; ++i) {
        fprintf(fPtr, "%s%x%s%08x\n", issInputMem1, bbPcPtr[i], issInputMem2, bbOpcPtr[i]);
    }
